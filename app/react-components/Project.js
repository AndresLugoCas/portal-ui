        ...
        <span style={{ ...styles.column, ...styles.margin }}>
          <EntityPageVerticalTable
            ...
            thToTd={[
              ...
              { th: `Disease Type${project.disease_type.length > 1 ? 's' : ''}`, td: project.disease_type.map(p => <div>{p}</div>) },
              { th: `Primary Site${project.primary_site.length > 1 ? 's' : ''}`, td: project.primary_site.map(p => <div>{p}</div>) },
